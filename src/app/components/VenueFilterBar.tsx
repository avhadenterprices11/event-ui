...
export function VenueFilterBar() {
  ...
  return (
    <motion.div
      ...
      className="relative z-20 -mt-24 md:-mt-32 max-w-[1440px] mx-auto px-6 md:px-12 lg:px-20"
    >
      <div className="bg-[#0B0B0D]/80 backdrop-blur-2xl border border-white/10 rounded-[24px] p-6 lg:p-8 shadow-[0_20px_40px_rgba(0,0,0,0.4)]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 items-end">
            ...
            <div className="flex bg-[#111114] border border-white/10 rounded-xl p-1.5 h-[54px] relative">
              <button
                ...
                className={`flex-1 rounded-lg text-sm transition-all duration-300 font-light relative z-10 ${
                  venueType === 'all' ? 'text-[#0B0B0D]' : 'text-white hover:text-[#C6A75E]'
                }`}
              >
                 All
              </button>
              <button
                ...
                className={`flex-1 rounded-lg text-sm transition-all duration-300 font-light relative z-10 ${
                  venueType === 'indoor' ? 'text-[#0B0B0D]' : 'text-white hover:text-[#C6A75E]'
                }`}
              >
                 Indoor
              </button>
              <button
                ...
                className={`flex-1 rounded-lg text-sm transition-all duration-300 font-light relative z-10 ${
                  venueType === 'outdoor' ? 'text-[#0B0B0D]' : 'text-white hover:text-[#C6A75E]'
             }`}
              >
                 Outdoor
              </button>
              ...
              <div 
                ...
                style={{
                   ...
                   transform: `translateX(${venueType === 'all' ? '0' : venueType === 'indoor' ? '100%' : '200%'})`,
                   marginLeft: venueType === 'outdoor' ? '4px' : venueType === 'indoor' ? '2px' : '0px'
                }}
              />
            </div>
          </div>
          ...
          <div className="flex items-end">
            <button className="group w-full h-[54px] relative rounded-xl overflow-hidden shadow-[0_10px_30px_rgba(198,167,94,0.15)] flex items-center justify-center gap-3">
